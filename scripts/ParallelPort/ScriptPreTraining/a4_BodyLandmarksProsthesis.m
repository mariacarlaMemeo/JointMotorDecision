...
xlsxFile = [path_excel_to_save '\Paradigmi\BodyLandmarkProsthesis.xlsx'];%TAG=45
...
try
    ...
    writetable(T,xlsxFile,'Sheet','BodyLandmarkProsthesis')%TAG=45
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
mkdir(PathID);
save([PathID '\' id '_BLProsthesis.mat'],'T')
Screen('CloseAll'); % or 'sca'