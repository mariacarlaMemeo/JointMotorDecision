%% PROSTHETICS: EMG Pattern FINGER
% In questo tipo di EMG pattern chiediamo al paziente di mantenere la
% posizione per 5 secondi (in quello sperimentale invece il movimento è
% continuo per 30 secondi di passaggio da un movimento all'altro ad esempio
% aprendo e chiudendo continuamento la mano). Quindi possiamo aumentare il
% numero di trials del doppio -> 10 per ogni condizione. E in questo caso
% facciamo muovere ogni singolo dito
...
n_training  = 0;
% Conditions
all_cond    = {'thumb' 'index' 'middle' 'ring' 'little'};
...
xlsxFile = [path_excel_to_save '\Paradigmi\EMGpatternFinger.xlsx'];
...
try
    writetable(T,xlsxFile,'Sheet','EMGpatternFinger')
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
save([PathID '\' id '_EMGpFinger.mat'],'T')