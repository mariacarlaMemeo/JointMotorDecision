...
vstart = [startStop_path '\RemoteStartStop.exe --start --name trial_' num2str(trialVicon, '%03i') ' --notes ' 'bla' ' --description ' 'blabla' ' --path C:/Temp'];
vstop = [startStop_path '\RemoteStartStop.exe --stop --name trial_' num2str(trialVicon, '%03i') ' --path "C:/Temp"'];
...
[SCREEN.windowPtr, SCREEN.rect]   = Screen('OpenWindow',SCREEN.screenID,[0 0 0],[1366 0 3290 1196]);
...
while ishandle( MessageBox )
    % done as we didnt understand stop and start
     trialVicon = 1;
...
%     vstop = [startStop_path '\RemoteStartStop.exe --stop --name trial_' num2str(trialVicon, '%03i') ' --path "C:/Temp"']
    ...
    if current > 0
        ...
        starting_frame{trial}(Counter) = MyClient.GetFrameNumber().FrameNumber;
        ...
        % fprintf( 'Index Pos[mm]: %s\n', index_pos(Counter).Translation(1));
        ...
        if not(index_pos(Counter).Occluded)
            ...
            if ~isempty(t0) && index_pos(Counter).Translation(1) <= x_high_threshold && index_pos(Counter).Translation(1)>= x_low_threshold && aperture(Counter) >= aperture_threshold
                large_frame(trial) = MyClient.GetFrameNumber().FrameNumber;
                t_large(trial) = (large_frame(trial) - starting_frame{trial}(1))/Output_GetFrameRate;%maybe (trial-1)    
                t1 = GetSecs();
...
%                 starting_frame(Counter) = MyClient.GetFrameNumber().FrameNumber;
            ...
            elseif ~isempty(t0) && index_pos(Counter).Translation(1) <= x_high_threshold && index_pos(Counter).Translation(1)>= x_low_threshold && aperture(Counter) <= aperture_threshold
                small_frame(trial) = MyClient.GetFrameNumber().FrameNumber;
                t_small(trial) = (small_frame(trial) - starting_frame{trial}(1))/Output_GetFrameRate;
                t1 = GetSecs();
                time_elapsed = 1000*(t1-t0);
