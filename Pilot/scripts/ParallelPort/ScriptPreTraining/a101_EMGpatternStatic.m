%% PROSTHETICS: EMG Pattern STATIC
...
% numero di trials del doppio -> 10 per ogni condizione
...
n_training  = 0;
% Conditions
all_cond    = {'Open' 'Close' 'Flexion' 'Extention' 'tridigital' 'rest' 'Index_pointing' 'Abduction'};
...
xlsxFile = [path_excel_to_save '\Paradigmi\EMGpatternStatic.xlsx'];
...
try
    writetable(T,xlsxFile,'Sheet','EMGpatternStatic')
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
save([PathID '\' id '_EMGpStatic.mat'],'T')