%% PROSTHETICS: EMG Pattern
% Subject perfoms different movement with intanct and phantom hand at the same time
...
n_training  = 0;
% Conditions
all_cond    = {'Open/Close' 'Flexion/Extention' 'tridigital' 'rest' 'Index_pointing' 'Adduction/Abduction'};
...
xlsxFile = [path_excel_to_save '\Paradigmi\EMGpattern.xlsx'];
...
try
    writetable(T,xlsxFile,'Sheet','EMGpattern')
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
