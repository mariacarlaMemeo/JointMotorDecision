%% PROSTHETICS: Postural Adjustment
% Subject reach and grasp an object at three different distances using the
% phantom limb
clear all
close all
...
n_training  = 0;
% Conditions
limbs       = {'intact limb' 'phantom limb'};
rand_limbs  = Shuffle(limbs);
all_cond    = {'near' 'center' 'far'};
...
for limb_condition = 1:length(rand_limbs)
    ...
    %Mostro il braccio sullo screen
    DrawFormattedText(window,(arm{:}),'center','center',[255 0 255]);
    Screen('Flip',window)
...
xlsxFile = [path_excel_to_save '\Paradigmi\PosturalAdjustment.xlsx'];
...
try
    writetable(T,xlsxFile,'Sheet','PosturalAdjustment')
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
