...
% Conditions
limbs       = {'intact'};%{'intact' 'phantom'};
rand_limbs  = Shuffle(limbs);
all_cond    = {'near_intact' 'far_intact'};
...
xlsxFile = [path_excel_to_save '\Paradigmi\HandBlinkReflex_Intact.xlsx'];
...
try
    ...
    writetable(T,xlsxFile,'Sheet','HandBlinkReflex_Intact');
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
save([PathID '\' id '_HBR_Intact.mat'],'T', 'threshold')
