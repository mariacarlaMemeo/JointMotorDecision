...
n_training = 1;
% Conditions
limbs       = {'phantom limb' 'intact limb'};
rand_limbs  = Shuffle(limbs);
all_cond_pha       = {'ulna' 'ulna' 'ulna'};
all_cond_intact    = {'ulna' 'ulna' 'ulna'};
...
for limb_condition = 1:length(rand_limbs)
    ...
    for training = 1:length(all_training)
        ...
        if strcmp(condition, 'tip of little')
            io64(ioObj,address,8);
        elseif strcmp(condition, 'tip of index')
            io64(ioObj,address,16);
        elseif strcmp(condition, 'tip of thumb')
            io64(ioObj,address,32);
        elseif strcmp(condition, 'ulna')
            io64(ioObj,address,64);
        elseif strcmp(condition, 'tip of stump')
            io64(ioObj,address,128);
        elseif strcmp(condition, 'elbow')
            io64(ioObj,address,128);
        end
        
        WaitSecs(1);
        io64(ioObj,address,4); %con questo apro il trigger direttamente dal digitimer
        WaitSecs(0.5);
        io64(ioObj,address,0);
...
xlsxFile = [path_excel_to_save '\Paradigmi\BodyLandmarkStimulator.xlsx'];%TAG=45
...
try
    ...
    writetable(T,xlsxFile,'Sheet','BodyLandmarkStimulator')%TAG=45
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
mkdir(PathID);
save([PathID '\' id '_BLStimulator.mat'],'T')
Screen('CloseAll'); % or 'sca'