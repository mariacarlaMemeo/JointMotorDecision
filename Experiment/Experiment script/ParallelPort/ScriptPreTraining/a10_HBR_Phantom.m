...
% Conditions
limbs       = {'phantom'};%{'intact' 'phantom'};
rand_limbs  = Shuffle(limbs);
all_cond    = {'near_phantom' 'far_phantom'};
...
xlsxFile = [path_excel_to_save '\Paradigmi\HandBlinkReflex_Phantom.xlsx'];
...
try
    ...
    writetable(T,xlsxFile,'Sheet','HandBlinkReflex_Phantom');
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
save([PathID '\' id '_HBR_phantom.mat'],'T', 'threshold')
