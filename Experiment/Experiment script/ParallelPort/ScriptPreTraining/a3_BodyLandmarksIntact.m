...
xlsxFile = [path_excel_to_save '\Paradigmi\BodyLandmarkIntact.xlsx'];%TAG=45
...
try
    ...
    writetable(T,xlsxFile,'Sheet','BodyLandmarkIntact')%TAG=45
    % erase the first 3 sheets created by default by excel
    RemoveSheet123(xlsxFile);
catch
    % The following function is to remove definitively the excel file
...
mkdir(PathID);
save([PathID '\' id '_BLIntact.mat'],'T')
Screen('CloseAll'); % or 'sca'