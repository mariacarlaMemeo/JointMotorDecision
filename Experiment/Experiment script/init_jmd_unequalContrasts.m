...
elseif data.isExperiment == 1 % experiment
    trialsInBlock = 44; % no. of trials for each agent taking first decision PER BLOCK
    blocksInRound = 2;  % 2 blocks
else
    error('isExperiment should be 0 or 1')
end

if mod(trialsInBlock,4) > 0
    error('Trial no. cannot be divided by 4.')
end


% CREATE TRIAL LIST: DETERMINE no. of CONTRASTS and assign to INTERVALS
if data.isExperiment == 0
    % 20 practice trials with FIXED number and order of target contrasts: 
    % 14 easy trials (contrast 1) and 2 trials for each remaining contrast
    % Note: the first 4 trials are for learning the procedure only.
    firstSecond = repmat(1:2,2,5);
    firstSecond = firstSecond(:);
    
    cs          = contrastSteps; % rename for brevity
    list_cs     = [repmat(cs(4),1,14), repmat(cs(3),1,2), repmat(cs(2),1,2) repmat(cs(1),1,2)]';
    list        = [firstSecond list_cs];
    conds_B     = list(1:2:end,:);
    conds_Y     = list(2:2:end,:);
    
elseif data.isExperiment == 1
    % List with 1s and 2s, to determine in which interval the oddball appears.
    % It should appear equally often in the 1st and 2nd interval.
    k=5;
    cs          = contrastSteps; % rename for brevity
    firstSecond = repmat(1:2,k,1); % 44 (trialsInBlock/2) trials: 22 per interval
    firstSecond = [firstSecond(:,1); firstSecond(:,2); 1;...
        firstSecond(:,1); firstSecond(:,2); 2;...
        firstSecond(:,1); firstSecond(:,2); 1;...
        firstSecond(:,1); firstSecond(:,2); 2];
    
    % List of target contrasts: NOTE that the 4 contrasts do not appear with
    % equal frequency.
    % 01 -> most difficult (low contrast), 04 -> most easy (high contrast)
    % 1=0.0150 (64x); 2=0.0350 (48x); 3=0.0700 (32x); 4=0.1500 (32x)
    % [1 2 3 4 1] %1°interval
    % [1 2 3 4 1] %2°interval
    % [1]         %1°interval
    % [1 2 3 4 2] %1°interval
    % [1 2 3 4 2] %2°interval
    % [1]         %1°interval
    % [1 2 3 4 1] %1°interval
    % [1 2 3 4 1] %2°interval
    % [1]         %2°interval
    ...
    % [1 2 3 4 2] %2°interval
    % [1]         %2°interval
    list_cs = [cs(1),cs(2),cs(3),cs(4),cs(1),...% first interval
        cs(1),cs(2),cs(3),cs(4),cs(1),...% second interval
        cs(1),...% first interval
        cs(1),cs(2),cs(3),cs(4),cs(2),...% first interval
        cs(1),cs(2),cs(3),cs(4),cs(2),...% second interval
        cs(1),...% first interval
        cs(1),cs(2),cs(3),cs(4),cs(1),...
        cs(1),cs(2),cs(3),cs(4),cs(1),...
        cs(1),...%second interval
        cs(1),cs(2),cs(3),cs(4),cs(2),...
        cs(1),cs(2),cs(3),cs(4),cs(2),...
        cs(1)];%second interval
    % Create trial list per agent per block and randomize in bins of 11
    % Randomization is done for pre-determined interval+contrast couples
    conds_B = [firstSecond list_cs']; % 44 rows, 2 columns (interval, contrast)
    conds_Y = [firstSecond list_cs'];
    v=11;
    cs_rand_B = [randperm(v), v+randperm(v),2*v+randperm(v),3*v+randperm(v)];
    conds_B = conds_B(cs_rand_B,:);
    cs_rand_Y = [randperm(v), v+randperm(v),2*v+randperm(v),3*v+randperm(v)];
    conds_Y = conds_Y(cs_rand_Y,:);
end
