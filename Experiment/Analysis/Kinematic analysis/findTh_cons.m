...
% -------------------------------------------------------------------------
% -> Here we find the moment(s) when the velocity threhold is passed.
% -------------------------------------------------------------------------
% This function is called from "movement_onset.m"
% Input arguments are: velocity vector (v), threhold (threshold), 
% no. of succesive samples that need to pass threhold (cons_ind)
% XXX currently we require 1+10 samples to pass threhold, i.e., 11 not 10
...
% strfind(diff(va),ones(1,cons_ind)) = [1 12]
% ind_va = [1 22] -> this gives you the index of the elements (which pass
